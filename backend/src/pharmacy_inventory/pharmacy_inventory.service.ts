  ...
  private readonly logger = new Logger(PharmacyInventoryService.name)
      ...
      async create(createPharmacyInventoryDto: CreatePharmacyInventoryDto): Promise<PharmacyInventory> {
        try {
          ...
          const savedInventory = await this.pharmacyInventoryRepository.save(inventory);
          return savedInventory;
        } catch (error) {
          this.logger.error('Failed to create new inventory', error.stack);
          throw new BadRequestException({message: 'Failed to create inventory.', error: error.message});
        }
      }
    ...
    try {
      ...
      this.logger.log(`Retrieved ${inventories.length} inventories successfully.`);
      return inventories;
    } catch (error) {
    ...
    async findOne(Inventory_id: number): Promise<PharmacyInventory> {
      try {
        const inventory = await this.pharmacyInventoryRepository.findOne({ where: {Inventory_id} });
        if(!inventory) {
    ...
    async update(Inventory_id: number, updatePharmacyInventoryDto: UpdatePharmacyInventoryDto): Promise <PharmacyInventory> {
      try {
        const inventory = await this.findOne(Inventory_id);
        Object.assign(inventory, updatePharmacyInventoryDto);
        const updatedInventory = await this.pharmacyInventoryRepository.save(inventory);
        this.logger.log(`Inventory with Id ${Inventory_id} updated successfully!`);
        return updatedInventory;
      } catch (error) {
        this.logger.error(`Failed to update inventory info with Id ${Inventory_id}`, error.stack);
    ...
    async delete(Inventory_id: number): Promise<{ message: string }> {
        try {
          ...
          this.logger.error(`Failed to delete inventory with ID: ${Inventory_id}`, error.stack);
