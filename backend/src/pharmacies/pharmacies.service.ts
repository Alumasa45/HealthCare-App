...
export class PharmaciesService {
  ...
  async pharmacistLogin(License_number: string) {
    try {
      ...
      if (!pharmacist) {
        this.logger.warn(
          `Pharmacist with license ${License_number} not found!`,
        );
  ...
  async findAll(): Promise<Pharmacy[]> {
    try {
      ...
      this.logger.log(
        `Retrieved ${pharmarcies.length} pharmacies successfully.`,
      );
      return pharmarcies;
    } catch (error) {
  ...
  async findByUserId(User_id: number): Promise<Pharmacy> {
    try {
      const pharmacy = await this.pharmacyRepository.findOne({
        where: { User_id },
      });
      if (!pharmacy) {
        this.logger.warn(`Pharmacy with User_id ${User_id} not found`);
        throw new NotFoundException('⚠ Pharmacy not found!');
      }
      this.logger.log(`Retrieved pharmacy for user ${User_id} successfully.`);
      return pharmacy;
    } catch (error) {
      this.logger.error(
        `Failed to retrieve pharmacy for user ${User_id}`,
        error.stack,
      );
      throw new BadRequestException('Failed to retrieve pharmacy for user.');
    }
  }
  ...
  async update(
    ...
    try {
      ...
      this.logger.error(
        `Failed to update pharmacy info with Id ${Pharmacy_id}`,
        error.stack,
      );
  ...
  async delete(Pharmacy_id: number): Promise<{ message: string }> {
    try {
      ...
      this.logger.error(
        `Failed to delete pharmacy with ID: ${Pharmacy_id}`,
        error.stack,
      );
  ...
  async findByLicenseNumber(License_Number: string): Promise<Pharmacy> {
    ...
    if (!pharmacist) {
      throw new NotFoundException(
        `Pharmacist with license number ${License_Number} not found`,
      );
    }
    return pharmacist;
  }
