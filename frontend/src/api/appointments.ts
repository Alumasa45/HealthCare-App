...
export const appointmentApi = {
    ...
    update: async (Appointment_id: number, appointmentData: Partial<AppointmentInfo>): Promise<Appointment> => {
        try {
            const response = await apiClient.patch(`/appointments/${Appointment_id}`, appointmentData);
            return response.data;
        } catch (error) {
            throw handleApiError(error);
        }
