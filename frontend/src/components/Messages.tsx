...
import { Send, Paperclip, Phone, Video, MoreVertical, Search, ArrowLeft, Circle, Check, CheckCheck, Clock, XCircle, Plus } from 'lucide-react';
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      {chat.lastMessage && (
                        <p className="text-sm text-gray-500 truncate">
                          {chat.lastMessage.type === 'file' ? '📎 File' : chat.lastMessage.content}
                        </p>
                      )}
                    </div>
          ...
          {message.type === 'file' ? (
            <div>
              <div className="flex items-center space-x-2 mb-1">
                ...
                <span className="text-sm font-medium">{message.attachmentName || 'File'}</span>
              </div>
...
const ChatWindow: React.FC<{
  ...
  const getStatusText = (user: User) => {
    ...
    return user.lastSeen ? `last seen ${user.lastSeen.toLocaleTimeString('en-US', { 
      hour: '2-digit', 
      minute: '2-digit' 
    })}` : 'offline';
  };
      ...
      <div className="bg-white border-b border-gray-200 p-4 flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">{getStatusText(participant)}</p>
          </div>
        </div>
...
function Messages() {
  const { user } = useAuth();
  const { chats, messages, selectedChatId, loading, error, selectChat, sendMessage, createNewConversation } = useChat();
      ...
      {selectedChat ? (
        <ChatWindow
          ...
          currentUser={{
            ...
            role: (user?.User_Type?.toLowerCase() as 'patient' | 'doctor' | 'pharmacist') || 'doctor',
            isOnline: true
          }}
